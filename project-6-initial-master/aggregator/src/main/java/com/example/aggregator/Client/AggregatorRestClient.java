...
public class AggregatorRestClient {
    ...
    {
        String uri = "http://localhost9091/getWordsThatContainConsecutiveLetters";

        ResponseEntity<Entry[]> responseEntity = restTemplate.getForEntity(uri, Entry[].class);
        Entry[] entryArray = responseEntity.getBody();

        return Arrays.stream(entryArray) //CONVERTS OBJ ARRAY TO LIST
                .collect(Collectors.toList());
    }

    public List<Entry> getWordsStartingWith(String chars)
    {
        String uri = "http://localhost9091/getWordsStartingWith/" + chars;
    ...
    {
        String uri = "http://localhost9091/getWordsEndingWith/" + chars;
